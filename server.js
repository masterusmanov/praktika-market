...
let categories_data = read_post('categories.json')
let subCategories_data = read_post('subCategories.json')
let products_data = read_post('products.json')
...
http.createServer((req, res) => {
    ...
    if (req.method == 'POST') {
        if (req_name == '/categories') {
            req.on('data', chunk => {
                ...
                write_post('categories.json', categories_data);
                res.writeHead(200, options);
        ...
        if (req_name == '/subcategories') {
            req.on('data', chunk => {
                ...
                write_post('subCategories.json', subCategories_data);
                res.writeHead(200, options);
        ...
        if (req_name == '/products') {
            req.on('data', chunk => {
                ...
                write_post('products.json', products_data);
                res.writeHead(200, options);
    ...
    if (req.method == 'DELETE') {
        ...
        if (req_name == '/products') {
            products_data.forEach((prod, indx) => {
                if (prod.product_id == req_id) {
                    products_data.splice(indx, 1);
                };
            });
            write_post('products.json', products_data);
            res.writeHead(200, options);
    ...
    console.log("server is running on 1987 port");
});
